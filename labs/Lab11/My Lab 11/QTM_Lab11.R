...
lm(amtWeekdays ~ gender + age, data = smoking) #use coefficients and then account for dummy variable in the equation, where male or female is accounted for.

fit_men<- lm(smoking$amtWeekdays ~ smoking$gender == 1 + smoking$age)
plot(fit_men, na.rm = T)
=======
# (1) What is the fitted model for men?
